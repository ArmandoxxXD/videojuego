   ...
   void Update()
{
    ...
    // Wall Slide detection (only if in the air and touching a wall)
    ...
    if (!m_grounded && (touchingWallRight || touchingWallLeft))
    {
        m_isWallSliding = true;

        // Block movement towards the wall only if still touching it
        if ((inputX > 0 && touchingWallRight) || (inputX < 0 && touchingWallLeft))
        {
            // The player is trying to move into the wall, stop horizontal movement
            inputX = 0;
        }
    }
    else
    ...
    else if (Mathf.Abs(inputX) > Mathf.Epsilon)
    {
        // Reset timer
        m_delayToIdle = 0.05f;
        m_animator.SetInteger("AnimState", 1);  // Run animation
    }
